...
public class DefiniteAssignmentVisitor extends Visitor {
    ...
    private HashMap<MethodOrValue, ControlBlock> tracked = new HashMap<MethodOrValue, ControlBlock>();
    ...
    public void visit(Tree.SpecifierStatement stmt) {
        ...
        if (bme instanceof Tree.MemberOrTypeExpression) {
            ...
            if (tracked.containsKey(decl) // non-variable and deferred
                ...
                if (elseBlock == null) {
                    ((MethodOrValue)decl).setSpecifiedInForElse(true);
                }
                ControlBlock assigningBlock = elseBlock != null ? elseBlock : forBlock;
                Set<MethodOrValue> assigned = assigningBlock.getSpecifiedValues();
                if (assigned == null) {
                    assigned = new HashSet<MethodOrValue>(1);
                    assigningBlock.setSpecifiedValues(assigned);
                }
                assigned.add((MethodOrValue)decl);
            }
        }
        super.visit(stmt);
    }
