...
public abstract class ClassOrPackageDoc extends CeylonDoc {
	...
	public ClassOrPackageDoc(final String destDir) {
		super(destDir);
	}

	protected String getModifiers(final Declaration decl) {
		final StringBuilder modifiers = new StringBuilder();
		if (decl.isShared()) {
			modifiers.append("shared ");
		}
		if (decl instanceof Value) {
			final Value value = (Value) decl;
			if (value.isVariable()) {
				modifiers.append("variable");
			}
		} else if (decl instanceof com.redhat.ceylon.compiler.typechecker.model.Class) {
			final com.redhat.ceylon.compiler.typechecker.model.Class klass  = (com.redhat.ceylon.compiler.typechecker.model.Class) decl;
			if (klass.isAbstract()) {
	...
	protected void doc(final Method method) throws IOException {
	...
	protected void doc(final MethodOrValue methodOrValue) throws IOException {
        open("tr class='TableRowColor'");
		open("td");
    ...
    protected void writeParameterList(final List<ParameterList> parameterLists) throws IOException {
	...
	protected String getDoc(final Declaration decl) {
		String doc = "";
